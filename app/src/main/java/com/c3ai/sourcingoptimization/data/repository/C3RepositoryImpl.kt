...
class C3RepositoryImpl @Inject constructor(private val api: C3ApiService) : C3Repository {
    ...
    override suspend fun getSupplierDetails(supplierId: String): C3Result<C3Supplier> = C3Result.on {
        api.getSupplierDetails(SupplierDetailsParameters(supplierId)).objs[0]
    }

    override suspend fun getSuppliedItems(supplierId: String): C3Result<List<C3Item>> = C3Result.on {
